...
const Dashboard = () => {
  ...
  <h1 className="text-3xl font-semibold">Hi {user?.name?.split(' ')[0] || 'there'}, welcome back!</h1>
        <p className="mt-3 max-w-2xl text-base text-white/80">
          Track your enrolled courses, compare new opportunities, and manage your certifications all in one sleek workspace.
        </p>
      ...
      <section>
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-slate-900">Your learning journey</h2>
          <a href="/courses" className="text-sm font-semibold text-primary hover:text-secondary">
            Discover more →
          </a>
        </div>
            ...
            <div key={enrollment.id} className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm">
              ...
              {enrollment.certificate && (
                <div className="mt-4 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-700">
                  ...
                  <a className="font-semibold text-emerald-700 underline" href={enrollment.certificate.url} target="_blank" rel="noreferrer">
                    View certificate
                  </a>
                </div>
              )}
